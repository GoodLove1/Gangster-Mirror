...
from hydrogram.filters import regex, command
from hydrogram.handlers import MessageHandler
from random import choice, random, randrange
from aiohttp import ClientSession as aClientSession
from hydrogram.types import BotCommand, CallbackQuery
...
from hydrogram.errors import PeerIdInvalid, RPCError, UserNotParticipant, FloodWait
...
async def command_listener(message, isClone=False, isGdrive=False, isJd=False, isLeech=False, isMega=False, isMirror=False, isQbit=False, isYtdl=False,isNzb=False):
    msg = ""
    tag = await get_tag(message)
    ...
    if message.from_user.id != OWNER_ID:
        ...
        elif isNzb and isLeech and not config_dict['NZB_ENABLED'] and not config_dict['LEECH_ENABLED']:
            msg = f"Hey {tag}.\n\nYoutube download and Leeching file in telegram both are disabled."
    ...
    if not limit_exceeded:
        if not isClone:
            if STORAGE_THRESHOLD := config_dict['STORAGE_THRESHOLD']:
                arch = any([listener.compress, listener.effmpegct])
